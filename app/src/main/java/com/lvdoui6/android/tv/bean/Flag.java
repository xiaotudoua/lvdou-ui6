package com.lvdoui6.android.tv.bean;
...
public class Flag implements Parcelable {
    ...
    public void createEpisode(String data) {
        ...
        for (int i = 0; i < urls.length; i++) {
            ...
            String name = split[0].isEmpty() ? number : split[0].trim();
            if (Utils.isBlockVideo(name)) continue;
            Episode episode = split.length > 1 ? Episode.create(name, split[1]) : Episode.create(number, urls[i]);
