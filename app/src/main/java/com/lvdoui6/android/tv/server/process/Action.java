package com.lvdoui6.android.tv.server.process;
...
import com.lvdoui6.android.tv.server.Nano;
...
public class Action implements Process {
    ...
    public NanoHTTPD.Response doResponse(NanoHTTPD.IHTTPSession session, String path, Map<String, String> files) {
        ...
        switch (Objects.requireNonNullElse(params.get("do"), "")) {
            ...
            case "push":
                ...
                return Nano.success();
            case "setting":
                if (HawkCustom.get().getConfig("custom_depot", false)) {
                    return Nano.error("当前账户没有自定义仓库权限");
                }
                onSetting(params);
                return Nano.success();
            case "logon":
                onLogon(params);
                return Nano.success("推送成功,请观察页面提示");
            case "file":
                onFile(params);
                return Nano.success();
            case "refresh":
    ...
    private void onLogon(Map<String, String> params) {
        String pass = params.get("pass");
        String account = params.get("account");
        if (TextUtils.isEmpty(pass) || TextUtils.isEmpty(account)) return;
        ServerEvent.logon(pass, account);
    }
